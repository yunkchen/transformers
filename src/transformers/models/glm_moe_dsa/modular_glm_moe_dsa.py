...
class GlmMoeDsaIndexer(nn.Module):
    ...
    **Cache strategy**: The Indexer manages its own key cache (`_cached_keys`) separately
    ...
    def forward(
        ...
        k_pe = apply_rotary_pos_emb(k_pe.unsqueeze(2), cos, sin, interleaved=rope_interleave, unsqueeze_dim=2).squeeze(2)  # [B, S, rope_D]
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        if is_prefill:
            ...
            attn_weights = torch.einsum(
                "bhsd,bhtd->bhst", query_states.float(), key_states.float()
            ) * self.scaling  # [B, H, S, T]

            # Build index_mask: -inf everywhere except selected top-k positions
            index_mask = torch.full(
                (batch_size, seq_length, total_len), float("-inf"),
                device=hidden_states.device, dtype=attn_weights.dtype,
            )
            ...
            # Build index_mask
            index_mask = torch.full(
                (batch_size, 1, total_len), float("-inf"),
                device=hidden_states.device, dtype=attn_weights.dtype,
            )
