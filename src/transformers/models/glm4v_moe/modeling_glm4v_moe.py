...
from ...utils import TransformersKwargs, auto_docstring, is_grouped_mm_available, logging, torch_compilable_check
...
class Glm4vMoeDecoderLayer(GradientCheckpointingLayer):
    def __init__(self, config: Glm4vMoeTextConfig, layer_idx: int):
        super().__init__()
        self.hidden_size = config.hidden_size
        ...
        self.self_attn = Glm4vMoeAttention(config, layer_idx)

        self.mlp = Qwen2MLP(config)
        self.input_layernorm = Glm4vMoeRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.post_attention_layernorm = Glm4vMoeRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
...
class Glm4vMoeTextModel(Glm4vMoePreTrainedModel):
    ...
    _can_record_outputs = {
        "hidden_states": Glm4vMoeDecoderLayer,
        "attentions": Glm4vMoeAttention,
    }
